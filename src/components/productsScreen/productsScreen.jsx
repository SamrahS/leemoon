...
function ProductsScreen() {
	...
	const settings = {
		...
		beforeChange: (next, current) => {
			if (current !== next) {
				const rotationProps = {
					rotate: `${(current > next ? '-' : '+')}${window.innerWidth > 768 ? 180 : 50}`,
					duration: 0.6
				};
		...
		<div className={Styles.mainScreen}>
			...
			<div className={Styles.bgContainer}>
				<div className={Styles.bgContent}>
					<button className={`mx-2 mb-1 ${Styles.addButton}`} onClick={() => updateCart(selectedIndex, true)}>Add to bag</button>
								...
								cart.map((item) => (
									item.count
										? <div className={Styles.cartItem} key={item.index}>
											...
											<img className={Styles.cartImage} src={item.img} alt="cartItem" />
											<p
												className={Styles.removeButton}
												onClick={() => updateCart(item.index, false)}>-</p>
										</div>
										: null
			...
			<Slider {...settings} ref={slider}>
				<div className={Styles.section}>
					<img id="product0" src={Tart} alt="tart" />
				</div>
				<div className={Styles.section}>
					<img id="product1" src={Tart} alt="tart" />
				</div>
				<div className={Styles.section}>
					<img id="product2" src={Tart} alt="tart" />
				</div>
				<div className={Styles.section}>
					<img id="product3" src={Tart} alt="tart" />
				</div>
			</Slider>
		</div>
	);
